    ...
    <Disclosure as="nav" class="bg-gray-800" v-slot="{ open }">
      ...
      <DisclosurePanel class="md:hidden">
        <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
          <router-link v-for="item in navigation" :key="item.name" :to="item.to" active-class='bg-gray-900 text-white' :class="[item.current ? 'this.$route.name === item.to.name' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'block rounded-md px-3 py-2 text-base font-medium']">{{ item.name }}</router-link>
        </div>
        <div class="border-t border-gray-700 pb-3 pt-4">
          ...
          <div class="mt-3 space-y-1 px-2">
            <DisclosureButton  as="a" class=" cursor-pointer block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white" @click="logout">Sign out</DisclosureButton>
          </div>
        </div>
      </DisclosurePanel>
    </Disclosure>
